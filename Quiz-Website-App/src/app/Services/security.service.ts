...
export class SecurityService {
  ...
  ifStillValid() : Observable<ProjectDetailsInterface> {  // checks if the token from localstorage is valid or not
    let ltoken : string = localStorage.getItem("quizToken") || this.token.Jwt;
  ...
  getProjectDetails() : ProjectDetails{
    ...
    this.user.name = nameArray[0] + " " + nameArray[1][0] + ".";
    this.user.logStatus = this.getLoginStatus();
    return this.projectDetails;
  }
  
  syncAll(username : string, pname : string) {
    if(localStorage.getItem("quizToken")==null && this.token.Jwt!=""){
      localStorage.setItem("quizToken", this.token.Jwt);
    }
    else if(this.token.Jwt=="" && localStorage.getItem("quizToken")!=null){
      this.token.Jwt = localStorage.getItem("quizToken") || "";
    }
    else{
      localStorage.setItem("quizToken", this.token.Jwt);
    }
